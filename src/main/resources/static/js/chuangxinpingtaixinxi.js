...
var sorts;
var id;
var platform_number2;
var platform_number;
var reporting_unit_court;

var totalPages;
var moduleName='pingtaixinxi';
        ...
        success: function (data) {
            ...
            if (data['status'] === 100) {
                ...
                $.each(data['value'], function (index, item) {
                    ...
                    option.className = "selectdiv fl";
                    option.value = id;
                    option.innerHTML = '<a> 平台分类：' + sorts + '</a>';
                    select.insertBefore(option, select.lastChild);
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data['status'] === 100) {
                ...
                showpingtaixinxi(1, cxptpagesize, firstGaoxiaoId);
            } else if (data['status'] === 200) {
                mui.toast("无更多内容了");
            } else {
                mui.toast("获取数据出错！\n数据获取出现错误，请联系系统维护人员处理。");
...
function showpage(identity) {
    //初始化成果页码
    cxptpage = 1;
    showpingtai(identity);
    showpingtaixinxi(cxptpage, cxptpagesize, identity);
    gettotalPages(moduleName,ord_id,pageSize);//获取成果页码
}

//存储获取的identity
function showpingtai(identity) {
...
function showdetails(ident) {
    ...
    sessionStorage.setItem("INF-identity", ident);
    // alert(ident);
    window.location.href = "/cxptxq?identity=" + sessionStorage.getItem("INF-identity");
}


//平台信息条目加载
function showpingtaixinxi(tpage, tpagesize, gxidentity) {
    var ul = document.body.querySelector('#pingtai_name');
    ul.innerHTML = "<li></li>";

    $.ajax({
        type: 'post',
        url: "/api/innovateplat",
        timeout: 15000,
        dataType: "json",
        data: {
            page: tpage,
        ...
        success: function (data) {
            if (data['status'] === 100) {
                ...
                $.each(data['value'], function (index, item) {
                    ...
                    platform_number2 = "'" + item['platform_number'] + "'";

                    var li = document.createElement('li');

                    li.innerHTML = '<a onclick="showdetails(' + platform_number2 + ')"><h2 class="one_hidden fs-14 fl" style="width: 70%;background-color:rgba(255,255,255,0);">' + name + '</h2>' +
                        '<p class="one_hidden fr fs-14" style="width: 20%;background-color:rgba(255,255,255,0);background-color:rgba(255,255,255,0);">' + reporting_unit_court + '</p></a>';
